...
import com.prueba.CRM.application.comment.services.querys.SearchCommentByIdQueryService;
import com.prueba.CRM.application.comment.validators.querys.SearchCommentByIdQueryValidator;
...
@Slf4j
@Component
public class SearchCommentByIdQueryHandler implements Command.Handler<SearchCommentByIdQuery, StatusResponse> {

    @Autowired
    private SearchCommentByIdQueryValidator validator;
    @Autowired
    private SearchCommentByIdQueryService service;

    @Override
    public StatusResponse handle(SearchCommentByIdQuery query) {
        ...
        try {
            statusResponse = this.service.searchCommentById(query);
