...
import com.prueba.CRM.application.contact.services.commands.AddContactCommadService;
import com.prueba.CRM.application.contact.validators.commands.AddContactCommandValidator;
...
@Slf4j
@Component
public class AddContactCommandHandler implements Command.Handler<AddContactCommand, StatusResponse> {

    @Autowired
    private AddContactCommandValidator validator;
    @Autowired
    private AddContactCommadService service;

    @Override
    public StatusResponse handle(AddContactCommand command) {
        ...
        try {
            statusResponse = this.service.addContact(command);
